...
public class JobOfferInitializer implements CommandLineRunner {
    ...
    public void run(String... args) throws Exception {
        List<CompanyAdvertiser> advertisers = companyAdvertiserRepository.findAll();
        ...
        List<JobOffer> jobOffers = List.of(
                ...
                new JobOffer("Description de l'offre 1", 30000, 50000, LocalDateTime.now(), 30, LocalDateTime.now().plusDays(30), true, advertisers.get(0), contractTypes.get(0), jobFunctions.get(0)),
                new JobOffer("Description de l'offre 2", 25000, 45000, LocalDateTime.now(), 30, LocalDateTime.now().plusDays(30), true, advertisers.get(1), contractTypes.get(1), jobFunctions.get(1)),
                new JobOffer("Description de l'offre 3", 35000, 60000, LocalDateTime.now(), 30, LocalDateTime.now().plusDays(30), true, advertisers.get(2), contractTypes.get(2), jobFunctions.get(2))
